locals {
  ...
  tf_remote_state_bucket = "dil-prod-v2-terraform-state"
...
module "cloudfront" {
  ...
  bucket_force_destroy = false # Keep backups in production
}

module "ecs_fargate" {
  source           = "../modules/ecs_fargate"
  name             = "${local.tenant_name}-ai-tutor-service"
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        { name = "REDIS_URL", value = local.enable_redis ? "rediss://prod-user:RedisSecurePassword2024!@${module.memorydb[0].cluster_endpoint}:6379" : "redis://localhost:6379" },
        { name = "REDIS_HOST", value = local.enable_redis ? module.memorydb[0].cluster_endpoint : "localhost" },
        { name = "REDIS_PORT", value = "6379" },
        { name = "REDIS_USERNAME", value = local.enable_redis ? "prod-user" : "" },
        ...
        { name = "TASK_VERSION", value = "v2.0-4vcpu-24gb-20251014" }
      ]
  ...
  description                        = "This is ${local.tenant_name} Production"
  enabled                            = true
  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        ...
        # Task definition version identifier to force updates
        { name = "TASK_VERSION", value = "v1.0-db-mcp-20251014" }
      ]
    }
  ])
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          "arn:aws:logs:us-east-2:*:log-group:/ecs/${local.tenant_name}-ai-tutor-service*"
        ]
      }
    ]
  })
...
locals {
  container_name = "${local.tenant_name}-ai-tutor-container"
...
resource "aws_ecs_cluster" "ecs-cluster" {
  name = "${local.tenant_name}-ecs-fargate-cluster"
}
...
module "alb" {
  ...
  internal                    = false
  idle_timeout                = 300 # Increased timeout for production
  enable_http2                = false
  ...
  target_type                      = "ip"
  deregistration_delay             = 300 # Reduced from 600 for faster deployments
  slow_start                       = 0
...
module "s3_lb_log" {
  ...
  force_destroy         = false # Keep logs in production
}
...
module "memorydb" {
  ...
  description            = "MemoryDB cluster for AI Tutor Backend Production"
  node_type              = "db.t4g.medium" # Upgraded for production
  port                   = 6379
  subnet_ids             = module.vpc.private_subnet_ids
  security_group_ids     = [aws_security_group.memorydb_sg[0].id]
  engine_version         = "7.0"
  num_shards             = 1
  num_replicas_per_shard = 2 # Increased replicas for production
  user_name              = "prod-user"
